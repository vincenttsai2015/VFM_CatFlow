...
def train_image_epoch(train, model, loader, optimizer, lr_scheduler, warmup_scheduler, device, max_nodes, mu, sigma, ema, distribution, tau_sched, loss_function, accelerator):
    ...
    for batch in loader:
        ...
        batch_x, batch_y = batch_x.to(device), batch_y.to(device) # [B, H*W, 2], [B]
        ...
        x_1 = batch_x.float() # [B, H*W, 2]
        print(f'x_1: {x_1.shape}, t: {t.shape}')

        t = t.unsqueeze(-1).unsqueeze(-1) # [B, 1, 1]
        print(f't: {t.shape}')
        x_t, v_x = conditional_velocity(distribution, x_1, t, tau_t, mu, sigma) # [B, H*W, 2] , [B, H*W, 2]
        print(f'x_t: {x_t.shape}, v_x: {v_x.shape}')

        pred = model(x_t.float(), t.float()) # [B, H*W, 2]
